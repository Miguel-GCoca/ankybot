...
class PolicyRunner(Node):
    ...
    Observation order (must match training ObservationsCfg.PolicyCfg, 49 total):
        ...
        height_command      (1)  - from /height_command (Float32, metres); default 0.24
    ...
    def __init__(self):
        ...
        self.declare_parameter('default_height_command', 0.24)  # metres
        self.declare_parameter('use_height_command', True)  # False for 48-obs standard env
        ...
        self.use_height_command = bool(p('use_height_command').value)
        ...
        self.height_cmd = np.array(
            [float(p('default_height_command').value)], dtype=np.float32
        )
        ...
        self.create_subscription(Float32, p('height_command_topic').value, self.height_cmd_cb, 10)
    ...
    def height_cmd_cb(self, msg: Float32):
        self.height_cmd[0] = msg.data
    # ------------------------------------------------------------------
    def _build_observation(self) -> np.ndarray:
        ...
        if self.use_height_command:
            parts.append(self.height_cmd)  # 1  height_command (metres) — 49-obs env only
        parts += [
