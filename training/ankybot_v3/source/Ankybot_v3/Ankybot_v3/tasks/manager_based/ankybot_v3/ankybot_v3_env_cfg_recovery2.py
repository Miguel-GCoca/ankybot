...
# 2026-07-18: rewritten to be identical to ankybot_v3_env_cfg_recovery1.py except:
#   - reset_joints.position_range narrowed from +-1.2 rad to +-15deg (+-0.2618 rad)
#   - reset_base.pose_range gains roll/pitch +-15deg (+-0.2618 rad) tilt (rec1 has no roll/pitch variance)
#   - commands.base_velocity.ranges.lin_vel_x capped to (0.0, 0.6) (part of a project-wide
#     lin_vel_x<=0.6 cap applied across every env cfg, not rec2-specific)
# The previous draft of this file (unregistered, structurally divergent from rec1 -- different
# action scale, reward set, reset ranges) was discarded rather than patched, since "identical to
# rec1 except X/Y" is only guaranteed by starting from rec1's actual content.
...
class RewardsCfg:
    ...
    # diagnostic only (weight negligible, does not meaningfully affect training) -- front/rear split
    # of foot_clearance_reward to check whether rear feet specifically are undershooting target_height
    # vs front. NOTE: RewardManager.compute() skips any term with weight exactly 0.0 (never calls the
    # func, never logs it) -- must use a tiny nonzero weight instead to actually get a reading.
    ...
    # diagnostic only (weight negligible, does not meaningfully affect training) -- front/rear split
    # of foot_slip to check whether rear feet specifically are sliding vs front. Same weight==0.0
    # skip-caveat as the clearance diagnostics above applies here too.
