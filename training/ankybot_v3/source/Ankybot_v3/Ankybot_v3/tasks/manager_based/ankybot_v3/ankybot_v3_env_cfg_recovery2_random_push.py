...
# 2026-07-19: rewritten to apply the same rec1->rec2 transform to
# ankybot_v3_env_cfg_recovery1_random_push.py (rand1-push): identical to
# rand1-push except:
...
#     rand1-push's values, matching recovery2_random.py's same widening
#     (mass_distribution_params (1.0,1.5)->(1.0,1.6), com_range ~20% wider on each axis)
# push_robot is unchanged from rand1-push. The previous draft of this file (registered but
# structurally divergent from rand1-push -- different action scale, reward set, reset ranges,
# same style as the old recovery2.py/recovery2_random.py drafts) was discarded rather than
# patched, for the same reason as those two files' rewrites -- see recovery2.py's header.
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        joint_pos = ObsTerm(
            func=mdp.joint_pos,
            noise=Unoise(n_min=-0.065, n_max=0.065),
        )
        actions = ObsTerm(
