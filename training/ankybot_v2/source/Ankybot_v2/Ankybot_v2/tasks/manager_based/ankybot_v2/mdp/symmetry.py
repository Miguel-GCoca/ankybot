...
def _transform_policy_obs(env: ManagerBasedRLEnv, obs: torch.Tensor, sym_name: str) -> torch.Tensor:
    """Apply one of 'lr', 'fb', 'diag' symmetry transforms to the 48-d policy obs."""
    slices = _policy_term_slices(env)
    if obs.shape[-1] != 48:
        raise ValueError(f"Ankybot V2 symmetry expected 48 policy observations, received {obs.shape[-1]}")
