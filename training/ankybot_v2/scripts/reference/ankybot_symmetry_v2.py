...
"""Left-right, front-back, and diagonal symmetry augmentation for Ankybot V2.

Geometric basis (FinalURDF_ANYmalConvention.urdf, verified in sim):
    - All four Hip joints rotate +X for +theta (unified across legs)
      -> Hip ("HAA-equivalent") is negated for LEFT-RIGHT and DIAGONAL,
         but just swapped (no sign change) for FRONT-BACK.
    - Front (FR/FL) Thigh+Foot rotate +Y; Rear (BR/BL) Thigh+Foot rotate -Y
      -> Thigh/Foot ("HFE/KFE-equivalent") is negated for FRONT-BACK and
         DIAGONAL, but just swapped (no sign change) for LEFT-RIGHT.

This matches ANYmal's own symmetry convention exactly (see
anymal_symmetry.py: HAA negated for LR, HFE/KFE negated for FB, both
negated for diagonal). 4x augmentation total (original + LR + FB + diag).
"""
...
def _remap_joint_name(name: str, leg_map: dict[str, str]) -> str:
    ...
    if not sep or leg not in leg_map:
        raise ValueError(f"Unsupported Ankybot V2 joint name: {name!r}")
...
def _build_permutation_and_signs(joint_names: Sequence[str], leg_map: dict[str, str], sym_name: str) -> tuple[list[int], torch.Tensor]:
    if len(joint_names) != 12 or len(set(joint_names)) != 12:
        raise ValueError(f"Expected 12 unique Ankybot V2 joint names, received {list(joint_names)!r}")

    # which joint TYPE gets negated for this symmetry transform
    #   lr   -> negate hip only
    #   fb   -> negate thigh/foot only
    #   diag -> negate both
    negate_types = {
        "lr": {"hip"},
        "fb": {"thigh_foot"},
...
def _policy_term_slices(env: ManagerBasedRLEnv) -> dict[str, slice]:
    def build(env):
        ...
        if term_names != _POLICY_TERMS:
            raise ValueError(f"Ankybot V2 symmetry expected policy terms {_POLICY_TERMS}, received {term_names}")
        import math
        ...
        if widths != expected_widths:
            raise ValueError(f"Ankybot V2 symmetry expected policy term widths {expected_widths}, received {widths}")
        slices = {}
        offset = 0
...
def _transform_policy_obs(env: ManagerBasedRLEnv, obs: torch.Tensor, sym_name: str) -> torch.Tensor:
    """Apply one of 'lr', 'fb', 'diag' symmetry transforms to the 45-d policy obs."""
    slices = _policy_term_slices(env)
    if obs.shape[-1] != 45:
        raise ValueError(f"Ankybot V2 symmetry expected 45 policy observations, received {obs.shape[-1]}")
    ...
    out[..., slices["base_ang_vel"]] = obs[..., slices["base_ang_vel"]] * ang_vel_sign
...
def _transform_actions(env: ManagerBasedRLEnv, actions: torch.Tensor, sym_name: str) -> torch.Tensor:
    _, action_remap = _runtime_remaps(env)[sym_name]["action"], None  # placeholder for clarity
